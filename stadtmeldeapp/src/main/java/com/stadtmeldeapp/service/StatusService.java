...
public class StatusService {
    ...
    public StatusEntity createStatus(StatusEntity status, HttpServletRequest request)
            throws NotFoundException, NotAllowedException {
        ReportingLocationEntity isAdminForLocation = userService.getUserFromRequest(request).getAdminForLocation();
        if (!status.getReportingLocationEntity().equals(isAdminForLocation))
            throw new NotAllowedException("Keine Berechtigung");
        return statusRepository.save(status);
    }

    public StatusEntity updateStatus(int id, StatusEntity updatedStatus, HttpServletRequest request)
            throws NotFoundException, NotAllowedException {
        ...
        ReportingLocationEntity isAdminForLocation = userService.getUserFromRequest(request).getAdminForLocation();
        if (!updatedStatus.getReportingLocationEntity().equals(isAdminForLocation))
            throw new NotAllowedException("Keine Berechtigung");
        return statusRepository.save(status);
    }
    ...
    public void deleteStatus(int id, HttpServletRequest request) throws NotFoundException, NotAllowedException {
        ...
        if (!statusLocation.equals(isAdminForLocation))
            throw new NotAllowedException("Keine Berechtigung");
